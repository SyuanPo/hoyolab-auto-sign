...
function postWebhook(hoyolabResp) {

  if (discord_notify && discordWebhook) {
    postDiscord(hoyolabResp);
  }

  if (telegram_notify && telegramBotToken && myTelegramID) {
    postTelegram(hoyolabResp);
  }
  if(line_notify && lineToken) {
    postLine(hoyolabResp);
  }
}

function postDiscord(data) {
